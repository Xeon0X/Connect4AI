...
def statAlphaBeta(depth: int):
    ...
    """
    depth_data = [{"depth": i+1, "nbWin": 0, "nbLoose": 0, "nbDraw":0, "win": [], "loose": [], "draw": []} for i in range(depth)]
    ...
    for depth1 in range(depth-1):
        for depth2 in range(depth1+1,depth):
            print(f"---------------------------------")
            print(f"Games started")
            print(f"Depth IA1: {depth1+1}")
            print(f"Depth IA2: {depth2+1}")
